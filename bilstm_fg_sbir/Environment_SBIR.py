...
class Environment():
    def __init__(self):
        ...
        for i_batch, sanpled_batch in enumerate(dataloader_sketchy_train):
            ...
            # shape of sanpled_batch['sketch_img']: [25, N, 3, 299, 299]
            for data_sketch in sanpled_batch['sketch_img']:
                sketch_feature = backbone_Network(data_sketch.to(device)) # [N, 2048]
                sketch_feature_ALL = torch.cat((sketch_feature_ALL, sketch_feature.detach())) # [25xN, 64]
            self.Sketch_Name_Train.extend(sanpled_batch['sketch_path'])
            self.Sketch_Array_Train.append(sketch_feature_ALL.cpu()) # list([25xN, 64])

            if sanpled_batch['positive_path'][0] not in self.Image_Name_Train:
                rgb_feature = model_fixed(sanpled_batch['positive_img'].to(device)) # [N, 64]
                self.Image_Array_Train = torch.cat((self.Image_Array_Train, rgb_feature.detach())) # [N, 64]
        ...
        dataloader_sketchy_test = data.DataLoader(dataset_sketchy_test, batch_size=test_opt.batch_size,
                                                  ...
                                                  num_workers=int(test_opt.nThreads)) # (25, N, 3, 299, 299)
        ...
        for i_batch, sanpled_batch in enumerate(dataloader_sketchy_test):
            # sanpled_batch['sketch_img'].shape = (25, N, 3, 299, 299)
            sketch_feature_ALL = torch.FloatTensor().to(device)
            for data_sketch in sanpled_batch['sketch_img']: 
                sketch_feature = backbone_Network(data_sketch.to(device)) # (N, 64)
                sketch_feature_ALL = torch.cat((sketch_feature_ALL, sketch_feature.detach())) # (25xN, 64)
            self.Sketch_Name_Test.extend(sanpled_batch['sketch_path'])
            self.Sketch_Array_Test.append(sketch_feature_ALL.cpu()) # list([25xN, 64])
