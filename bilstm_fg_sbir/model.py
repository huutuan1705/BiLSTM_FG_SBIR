    ...
    def evaluate(self, dataloader_test):
        ...
        for i_batch, sanpled_batch in enumerate(sketch_array_tests):
            ...
            for i_sketch in range(sanpled_batch.shape[0]):
                ...
                # print("shape sketch_feature[:, -1, :]: ", sketch_feature[:, -1, :].shape) #[1, 64]
                
                target_distance = F.pairwise_distance(sketch_feature[:, -1, :][-1].unsqueeze(0).to(device), image_array_tests[position_query].unsqueeze(0).to(device))
                distance = F.pairwise_distance(sketch_feature[:, -1, :][-1].unsqueeze(0).to(device), image_array_tests.to(device))
